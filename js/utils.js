...
// CORREGIDO: Fusionada tabla de máscaras default en tabla de clases.
// ==================================================
...
function formatNumber(num) { try { const lang = document.documentElement.lang || 'es'; return num.toLocaleString(lang); } catch (e) { return num.toString(); } }
...
/**
 * Genera tabla HTML de rangos de clases IP, incluyendo máscara por defecto.
 * @param {string} [highlightClass=null] - La clase a resaltar en la tabla.
 * @returns {string} El string HTML de la tabla.
 */
export function generateClassRangeTableHTML(highlightClass = null) {
    const classData = [
        { class: 'A', range: '1 - 126', mask: '255.0.0.0', note: getTranslation('explanation_class_range_note_a') || '(Nota: 127.x.x.x es Loopback)'},
        { class: 'B', range: '128 - 191', mask: '255.255.0.0', note: '' },
        { class: 'C', range: '192 - 223', mask: '255.255.255.0', note: '' },
        { class: 'D', range: '224 - 239', mask: 'N/A', note: getTranslation('class_note_multicast') || '(Multicast)' },
        { class: 'E', range: '240 - 255', mask: 'N/A', note: getTranslation('class_note_experimental') || '(Experimental)' }
    ];
    // Usar la clave existente para la introducción
    let tableHTML = `<p>${getTranslation('explanation_class_mask_intro')}</p>`;
    tableHTML += '<table class="explanation-table">';
    // Añadir la cabecera para la máscara por defecto
    ...
    classData.forEach(item => {
        ...
        // Añadir la celda para la máscara
        tableHTML += `<tr${highlight}><td>${item.class}</td><td>${item.range}</td><td><code>${item.mask}</code></td><td>${item.note || ''}</td></tr>`;
    });
    tableHTML += '</tbody></table>';
    // Ya no necesitamos la nota específica de clase A aquí, está en la tabla.
    return tableHTML;
}

// --- generateClassMaskTableHTML ya no es necesaria y se puede eliminar ---
...
/** Genera explicación HTML para cálculo de Wildcard (solo resta visual). */
...
export function generateSubnettingExplanationHTML(ip, mask, networkAddr, broadcastAddr, usableHosts, numSubnets, originalClassMask) {
    try {
        ...
        andingHtml += `<p style="text-align: center; font-family: monospace; font-size: 0.9em; margin-top: 5px; margin-bottom: 0;">${getTranslation('explanation_network_address_decimal')} <code>${networkAddr}</code></p>`;
        andingHtml += `</div>`;
        ...
        // 3. Explicación paso a paso para Hosts Utilizables
        let hostCalcHtml = '';
        ...
        // 4. Tabla principal con otros resultados (Broadcast)
        let resultsTableHtml = '<table class="explanation-table" style="margin-top: 15px;">';
        ...
        // Combinar todo
        html += andingHtml + subnetCalcHtml + hostCalcHtml + resultsTableHtml;
