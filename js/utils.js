...
export function calculateBroadcastAddress(ipString, maskString) { try { const ipOctets = ipString.split('.').map(Number); const maskOctets = maskString.split('.').map(Number); if (ipOctets.length !== 4 || maskOctets.length !== 4 || ipOctets.some(isNaN) || maskOctets.some(isNaN) || ipOctets.some(o => o < 0 || o > 255) || maskOctets.some(o => o < 0 || o > 255) ) { throw new Error("Formato IP o máscara inválido para cálculo de broadcast."); } const broadcastOctets = []; for (let i = 0; i < 4; i++) { const wildcardOctet = maskOctets[i] ^ 255; broadcastOctets.push(ipOctets[i] | wildcardOctet); } return broadcastOctets.join('.'); } catch (error) { console.error("Error calculando dirección de broadcast:", error); return null; } }

/**
 * NUEVA FUNCIÓN: Calcula la máscara wildcard a partir de una máscara de subred.
...
export function calculateWildcardMask(maskString) {
    try {
        ...
        const wildcardOctets = maskOctets.map(octet => 255 - octet); // Simple resta para wildcard
...
/**
 * REESCRITA: Genera el HTML para la explicación de cálculo de Red/Broadcast.
 * Muestra IP, Máscara, Wildcard, Dir. Red y Dir. Broadcast en una tabla.
...
export function generatePortionExplanationHTML(ipString, maskString, wildcardString, networkAddr, broadcastAddr) {
    try {
        ...
        let html = `<p>Para la IP <strong>${ipString}</strong> y la Máscara <strong>${maskString}</strong>:</p>`;
        html += '<table class="explanation-table">';
        html += '<thead><tr><th>Concepto</th><th>Valor</th><th>Cálculo</th></tr></thead>';
        html += '<tbody>';
        html += `<tr><td>IP Original</td><td><code>${ipString}</code></td><td>-</td></tr>`;
        html += `<tr><td>Máscara</td><td><code>${maskString}</code></td><td>-</td></tr>`;
        html += `<tr><td>Wildcard</td><td><code>${wildcardString}</code></td><td>(NOT Máscara)</td></tr>`;
        html += `<tr><td>Dir. Red</td><td><code>${networkAddr}</code></td><td>(IP AND Máscara)</td></tr>`;
        html += `<tr><td>Dir. Broadcast</td><td><code>${broadcastAddr}</code></td><td>(IP OR Wildcard)</td></tr>`;
        html += '</tbody></table>';
