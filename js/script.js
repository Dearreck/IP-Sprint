...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function getUserData(username) {
        ...
        if (allUserData[username]) {
             ...
             allUserData[username].associatePerfectStreak = allUserData[username].associatePerfectStreak || 0; // Añadido
            return allUserData[username];
        } else {
            return { unlockedLevels: ['Entry'], entryPerfectStreak: 0, associatePerfectStreak: 0 }; // Añadido
        }
     }
    ...
    function getIpInfo(ipString) { try { const octets = ipString.split('.').map(Number); const firstOctet = octets[0]; let ipClass = ''; let ipType = 'Pública'; let defaultMask = 'N/A'; if (firstOctet >= 1 && firstOctet <= 126) { ipClass = 'A'; defaultMask = '255.0.0.0'; } else if (firstOctet >= 128 && firstOctet <= 191) { ipClass = 'B'; defaultMask = '255.255.0.0'; } else if (firstOctet >= 192 && firstOctet <= 223) { ipClass = 'C'; defaultMask = '255.255.255.0'; } else if (firstOctet >= 224 && firstOctet <= 239) { ipClass = 'D'; ipType = 'N/A'; } else if (firstOctet >= 240 && firstOctet <= 255) { ipClass = 'E'; ipType = 'N/A'; } else if (firstOctet === 127) { ipClass = 'A'; ipType = 'Loopback'; defaultMask = '255.0.0.0'; } if (firstOctet === 10 || (firstOctet === 172 && octets[1] >= 16 && octets[1] <= 31) || (firstOctet === 192 && octets[1] === 168)) { if (ipType !== 'Loopback') ipType = 'Privada'; } return { class: ipClass, type: ipType, defaultMask: defaultMask }; } catch (error) { console.error("Error en getIpInfo con IP:", ipString, error); return { class: 'N/A', type: 'N/A', defaultMask: 'N/A' }; } }
    function shuffleArray(array) { for (let i = array.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [array[i], array[j]] = [array[j], array[i]]; } }

    // --- Generadores de Preguntas (Nivel Entry) ---
    function generateClassQuestion() { const ip = generateRandomIp(); const info = getIpInfo(ip); const question = `Dada la IP: <strong>${ip}</strong><br>¿A qué clase pertenece?`; const options = ['A', 'B', 'C', 'D', 'E']; correctAnswer = info.class; return { question, options }; }
    function generateTypeQuestion() { let ip, info, attempts = 0; do { ip = generateRandomIp(); info = getIpInfo(ip); attempts++; } while ((info.type === 'N/A' || info.type === 'Loopback') && attempts < 100); if(attempts >= 100) { ip = '8.8.8.8'; info = getIpInfo(ip); } const question = `Dada la IP: <strong>${ip}</strong><br>¿Es Pública o Privada?`; const options = ['Pública', 'Privada']; correctAnswer = info.type; return { question, options }; }
    function generateDefaultMaskQuestion() { let ip, info, attempts = 0; do { ip = generateRandomIp(); info = getIpInfo(ip); attempts++; } while ((info.class !== 'A' && info.class !== 'B' && info.class !== 'C') && attempts < 100); if(attempts >= 100) { ip = '192.168.1.1'; info = getIpInfo(ip); } const question = `Dada la IP: <strong>${ip}</strong> (Clase ${info.class})<br>¿Cuál es su máscara de subred por defecto?`; const options = ['255.0.0.0', '255.255.0.0', '255.255.255.0']; correctAnswer = info.defaultMask; return { question, options }; }
    function generateSelectClassQuestion() { const targetClasses = ['A', 'B', 'C']; const targetClass = targetClasses[getRandomInt(0, targetClasses.length - 1)]; const question = `¿Cuál de las siguientes IPs pertenece a la Clase <strong>${targetClass}</strong>?`; let correctIp = ''; let incorrectIps = []; let attempts = 0; let ipSet = new Set(); while (!correctIp && attempts < 100) { let ip = generateRandomIp(); if (getIpInfo(ip).class === targetClass) { correctIp = ip; ipSet.add(ip); } attempts++; } if (!correctIp) { if(targetClass === 'A') correctIp = '10.1.1.1'; else if(targetClass === 'B') correctIp = '172.16.1.1'; else correctIp = '192.168.1.1'; ipSet.add(correctIp); } attempts = 0; while (incorrectIps.length < 3 && attempts < 200) { let ip = generateRandomIp(); if (getIpInfo(ip).class !== targetClass && !ipSet.has(ip)) { incorrectIps.push(ip); ipSet.add(ip); } attempts++; } while (incorrectIps.length < 3) { let ip = generateRandomIp(); if(!ipSet.has(ip)) incorrectIps.push(ip); } const options = [correctIp, ...incorrectIps]; shuffleArray(options); correctAnswer = correctIp; return { question, options }; }
    function generateSelectPrivateIpQuestion() { const question = `¿Cuál de las siguientes direcciones IP es <strong>Privada</strong>?`; let correctIp = ''; let incorrectIps = []; let attempts = 0; let ipSet = new Set(); while (!correctIp && attempts < 100) { let ip = generateRandomIp(); if (getIpInfo(ip).type === 'Privada') { correctIp = ip; ipSet.add(ip); } attempts++; } if (!correctIp) { correctIp = '192.168.1.1'; ipSet.add(correctIp); } attempts = 0; while (incorrectIps.length < 3 && attempts < 200) { let ip = generateRandomIp(); if (getIpInfo(ip).type === 'Pública' && !ipSet.has(ip)) { incorrectIps.push(ip); ipSet.add(ip); } attempts++; } while (incorrectIps.length < 3) { let ip = generateRandomIp(); if(!ipSet.has(ip)) incorrectIps.push(ip); } const options = [correctIp, ...incorrectIps]; shuffleArray(options); correctAnswer = correctIp; return { question, options }; }
    function generateSelectIpByDefaultMaskQuestion() { const targetMasks = ['255.0.0.0', '255.255.0.0', '255.255.255.0']; const targetMask = targetMasks[getRandomInt(0, targetMasks.length - 1)]; const question = `¿Cuál de las siguientes IPs usaría la máscara por defecto <strong>${targetMask}</strong>?`; let correctIp = ''; let incorrectIps = []; let attempts = 0; let ipSet = new Set(); while (!correctIp && attempts < 100) { let ip = generateRandomIp(); if (getIpInfo(ip).defaultMask === targetMask) { correctIp = ip; ipSet.add(ip); } attempts++; } if (!correctIp) { if(targetMask === '255.0.0.0') correctIp = '10.1.1.1'; else if(targetMask === '255.255.0.0') correctIp = '172.16.1.1'; else correctIp = '192.168.1.1'; ipSet.add(correctIp); } attempts = 0; while (incorrectIps.length < 3 && attempts < 200) { let ip = generateRandomIp(); let info = getIpInfo(ip); if (info.defaultMask !== 'N/A' && info.defaultMask !== targetMask && !ipSet.has(ip)) { incorrectIps.push(ip); ipSet.add(ip); } attempts++; } while (incorrectIps.length < 3) { let ip = generateRandomIp(); if(!ipSet.has(ip)) incorrectIps.push(ip); } const options = [correctIp, ...incorrectIps]; shuffleArray(options); correctAnswer = correctIp; return { question, options }; }

    // --- Funciones UI / Flujo ---
    function updateUnlockProgressUI() {
        try {
             if (!currentUserData || !unlockProgressSection || !unlockProgressDiv || !progressStarsSpan) return;
             unlockProgressSection.style.display = 'block'; // Sección siempre visible post-login
             const unlocked = currentUserData.unlockedLevels || ['Entry'];
             const entryStreak = currentUserData.entryPerfectStreak || 0;
             const associateStreak = currentUserData.associatePerfectStreak || 0; // Cargar racha Associate
             ...
             if (!unlocked.includes('Associate')) {
                 ...
                 progressTitle = "Progreso para Nivel Associate:"; showProgress = true;
             } else if (!unlocked.includes('Professional')) {
                 targetLevel = 'Professional'; currentStreak = associateStreak;
                 progressTitle = "Progreso para Nivel Professional:"; showProgress = true;
    ...
    function loadNextQuestion() {
        feedbackArea.textContent = ''; if(optionsContainer) optionsContainer.classList.remove('options-disabled');
        ...
        try {
            if (currentLevel === 'Entry') {
                ...
                const randomIndex = getRandomInt(0, questionTypes.length - 1); const generatorFunction = questionTypes[randomIndex];
            ...
            else { throw new Error("questionData inválido generado"); }
        ...
        try {
            ...
            if (currentLevel === 'Entry') {
                if (isPerfect) {
                    ...
                    } else if (!currentUserData.unlockedLevels.includes('Associate')) { message = `¡Ronda Perfecta! Racha (Entry): ${currentUserData.entryPerfectStreak}/3.`; }
                     else { message = "¡Ronda Perfecta!"; }
                } else { if (currentUserData.entryPerfectStreak > 0) { console.log("Racha de Entry reiniciada."); } currentUserData.entryPerfectStreak = 0; message = "¡Partida completada!"; }
            } else if (currentLevel === 'Associate') { // Lógica racha Associate
                 if (isPerfect) {
                     ...
                     if (currentUserData.associatePerfectStreak >= 3 && !currentUserData.unlockedLevels.includes('Professional')) {
                         currentUserData.unlockedLevels.push('Professional'); currentUserData.associatePerfectStreak = 0;
                         message = "¡3 Rondas Perfectas! ¡Nivel Professional Desbloqueado! 🏆";
                     } else if (!currentUserData.unlockedLevels.includes('Professional')) { message = `¡Ronda Perfecta en Associate! Racha: ${currentUserData.associatePerfectStreak}/3.`; }
                      else { message = "¡Ronda Perfecta en Associate!"; }
                 } else { if (currentUserData.associatePerfectStreak > 0) { console.log("Racha de Associate reiniciada."); } currentUserData.associatePerfectStreak = 0; message = "¡Partida completada!"; }
            } else { message = "¡Partida completada!"; } // Para otros niveles o si no es Entry/Associate

            saveUserData(currentUsername, currentUserData); // Guardar TODOS los datos actualizados
            ...
            if(gameOverSection) gameOverSection.style.display = 'block'; if(unlockProgressSection) unlockProgressSection.style.display = 'block';
    ...
    function handleUserLogin(username) {
         ...
         try {
             ...
             if(highScoresSection) highScoresSection.style.display = 'block';
             if(unlockProgressSection) unlockProgressSection.style.display = 'block'; // Mostrar al loguear
             showLevelSelection();
         } catch (error) { console.error("Error durante handleUserLogin:", error); alert("Hubo un problema al cargar los datos del usuario."); /* ... (manejo error como antes) ... */ }
    }
    loadHighScores();
    ...
    if(levelSelectSection) levelSelectSection.style.display = 'none'; if(unlockProgressSection) unlockProgressSection.style.display = 'none';
    if(highScoresSection) highScoresSection.style.display = 'none'; if(gameAreaSection) gameAreaSection.style.display = 'none'; if(gameOverSection) gameOverSection.style.display = 'none';
