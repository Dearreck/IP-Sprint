...
export function displayQuestion(questionData, answerClickHandler) {
    try {
        ...
        questionData.options.forEach((optionData) => {
            ...
            // --- Lógica CORREGIDA para determinar texto y valor original ---
            if (typeof optionData === 'string') {
                // Intenta traducir el string como clave. Si no existe, usa el string tal cual.
                const translated = getTranslation(optionData);
                buttonText = (translated !== optionData) ? translated : optionData; // Usa traducción si existe, si no, el valor original
