...
function generateDefaultMaskQuestion() {
    try {
        ...
        const explanationInfo = { generatorName: 'generateClassMaskTableHTML', args: [info.class] };
...
function generateSelectClassQuestion() {
    try{
        ...
        const explanationInfo = { baseTextKey: 'explanation_select_ip_for_class', replacements: { targetClass: targetClass, correctIp: correct }, generatorName: 'generateClassRangeTableHTML', args: [targetClass] };
        return { question, options, correctAnswer: correct, explanation: explanationInfo };
...
function generateSelectPrivateIpQuestion() {
    try {
        ...
        shuffleArray(options);
        const correct = correctIp;
        const explanationInfo = { generatorName: 'generatePrivateRangeTableHTML', args: [correct] };
        return { question, options, correctAnswer: correct, explanation: explanationInfo };
...
function generateSelectIpByDefaultMaskQuestion() {
    try {
        ...
        const explanationInfo = { baseTextKey: 'explanation_select_ip_for_mask', replacements: { class: correctClass, mask: targetMask }, generatorName: 'generateClassMaskTableHTML', args: [correctClass] };
        return { question, options, correctAnswer: correct, explanation: explanationInfo };
...
function generateClassAndTypeQuestion() {
    try {
        ...
        const explanationInfo = { generators: [ { generatorName: 'generateClassRangeTableHTML', args: [correctClass] }, { generatorName: 'generatePrivateRangeTableHTML', args: [ip] } ], separator: '<hr style="margin: 10px 0;">' };
...
function generateClassAndDefaultMaskQuestion() {
    try {
        ...
        const explanationInfo = { generatorName: 'generateClassMaskTableHTML', args: [correctClass] };
        return { question, options, correctAnswer: correctAnswerObject, explanation: explanationInfo };
...
function generateClassAndNetworkPortionQuestion() {
    try {
        ...
        if (incorrectNetworkPortion && incorrectNetworkPortion !== correctNetworkPortion) { options.push({ classKey: `option_class_${correctClass.toLowerCase()}`, portionKey: 'option_network_portion', portionValue: incorrectNetworkPortion }); }
        else { let fallbackPortion = (correctClass !== 'A') ? `${getRandomInt(1,126)}` : `${getRandomInt(128,191)}.${getRandomInt(0,255)}`; options.push({ classKey: `option_class_${correctClass.toLowerCase()}`, portionKey: 'option_network_portion', portionValue: fallbackPortion }); }
        if (possibleClassesLower.length > 0) { options.push({ classKey: `option_class_${possibleClassesLower[0]}`, portionKey: 'option_network_portion', portionValue: correctNetworkPortion || 'None' }); }
        if (possibleClassesLower.length > 0 && incorrectNetworkPortion && incorrectNetworkPortion !== correctNetworkPortion) { options.push({ classKey: `option_class_${possibleClassesLower[0]}`, portionKey: 'option_network_portion', portionValue: incorrectNetworkPortion }); }
        let existingOptionsStr = new Set(options.map(o => `${o.classKey},${o.portionKey},${o.portionValue}`));
        while (options.length < 4) {
            const randomClassKey = `option_class_${['a', 'b', 'c'][getRandomInt(0, 2)]}`; // minúscula
            let randomPortion = ''; if (randomClassKey === 'option_class_a') randomPortion = `${getRandomInt(1, 126)}`; else if (randomClassKey === 'option_class_b') randomPortion = `${getRandomInt(128, 191)}.${getRandomInt(0, 255)}`; else randomPortion = `${getRandomInt(192, 223)}.${getRandomInt(0, 255)}.${getRandomInt(0, 255)}`;
        ...
        const explanationInfo = { generatorName: 'generatePortionExplanationHTML', args: [ip, info.defaultMask, wildcardMask, networkAddr, broadcastAddr] };
        return { question, options, correctAnswer: correctAnswerObject, explanation: explanationInfo };
...
function generateClassAndHostPortionQuestion() {
    try {
        ...
        if (incorrectHostPortion && incorrectHostPortion !== correctHostPortion) { options.push({ classKey: `option_class_${correctClass.toLowerCase()}`, portionKey: 'option_host_portion', portionValue: incorrectHostPortion }); }
        else { let fallbackPortion = (correctClass !== 'C') ? `${getRandomInt(1,254)}` : `${getRandomInt(0,255)}.${getRandomInt(1,254)}`; options.push({ classKey: `option_class_${correctClass.toLowerCase()}`, portionKey: 'option_host_portion', portionValue: fallbackPortion }); }
        if (possibleClassesLower.length > 0) { options.push({ classKey: `option_class_${possibleClassesLower[0]}`, portionKey: 'option_host_portion', portionValue: correctHostPortion || 'None' }); }
        if (possibleClassesLower.length > 0 && incorrectHostPortion && incorrectHostPortion !== correctHostPortion) { options.push({ classKey: `option_class_${possibleClassesLower[0]}`, portionKey: 'option_host_portion', portionValue: incorrectHostPortion }); }
        let existingOptionsStr = new Set(options.map(o => `${o.classKey},${o.portionKey},${o.portionValue}`));
        while (options.length < 4) {
            const randomClassKey = `option_class_${['a', 'b', 'c'][getRandomInt(0, 2)]}`; // minúscula
            let randomPortion = ''; if (randomClassKey === 'option_class_a') randomPortion = `${getRandomInt(0, 255)}.${getRandomInt(0, 255)}.${getRandomInt(1, 254)}`; else if (randomClassKey === 'option_class_b') randomPortion = `${getRandomInt(0, 255)}.${getRandomInt(1, 254)}`; else randomPortion = `${getRandomInt(1, 254)}`;
...
// ... (Resto de generadores de Associate y función getNextQuestion sin cambios respecto a la v3) ...
...
function generateRfc1918Question() {
    try {
        ...
        if (questionType === 2) { explanationInfo.baseTextKey = 'explanation_rfc1918_class_note'; explanationInfo.replacements = { blockId: `<strong>${chosenBlock.blockId}</strong>`, class: `<strong>${chosenBlock.class}</strong>` }; }
...
function generateSpecialAddressQuestion() {
    try {
        ...
        let chosenSpecialType = ''; let explanationInfo = {};

        if (pickSpecial) {
            const chosenSpecial = specialAddresses[getRandomInt(0, specialAddresses.length - 1)];
            targetIp = chosenSpecial.ip; correctAnswerKey = chosenSpecial.descriptionKey; chosenSpecialType = chosenSpecial.type;
            explanationInfo = { generatorName: 'generateSpecialAddressExplanationHTML', args: [chosenSpecialType] };

            if (questionFormat === 0) {
                const question = { key: 'question_special_what_type', replacements: { ip: `<strong>${targetIp}</strong>` } };
                let options = [correctAnswerKey]; let incorrectOptions = specialAddresses.filter(s => s.type !== chosenSpecial.type).map(s => s.descriptionKey); incorrectOptions.push(...normalIpTypeKeys); shuffleArray(incorrectOptions); options.push(...incorrectOptions.slice(0, 3)); shuffleArray(options); if (options.length > 4) options = options.slice(0, 4); if (!options.includes(correctAnswerKey)) { options.pop(); options.push(correctAnswerKey); shuffleArray(options); }
                ...
                let otherSpecial = specialAddresses.find(s => s.type !== chosenSpecial.type); if (otherSpecial) { incorrectIps.add(otherSpecial.ip); } else { while (incorrectIps.size < 3) { let ip = generateRandomIp(); if (getIpInfo(ip).type !== 'Loopback' && getIpInfo(ip).type !== 'APIPA') { incorrectIps.add(ip); } } }
                options.push(...Array.from(incorrectIps).slice(0, 3)); shuffleArray(options); if (options.length > 4) options = options.slice(0, 4); if (!options.includes(targetIp)) { options.pop(); options.push(targetIp); shuffleArray(options); }
            ...
            correctAnswerKey = `option_${info.type.toLowerCase()}`; chosenSpecialType = info.type;
            const question = { key: 'question_special_what_type', replacements: { ip: `<strong>${targetIp}</strong>` } };
            let options = [correctAnswerKey]; let incorrectOptions = specialAddresses.map(s => s.descriptionKey); shuffleArray(incorrectOptions); options.push(...incorrectOptions.slice(0, 3)); shuffleArray(options); if (options.length > 4) options = options.slice(0, 4); if (!options.includes(correctAnswerKey)) { options.pop(); options.push(correctAnswerKey); shuffleArray(options); }
            explanationInfo = { generatorName: 'generatePrivateRangeTableHTML', args: [targetIp] };
            return { question, options, correctAnswer: correctAnswerKey, explanation: explanationInfo };
        }
...
function generateNetworkBroadcastAddressQuestion() {
    try {
        ...
        let options = new Set([correctAnswer]);
        const otherAddress = askForNetwork ? broadcastAddr : networkAddr; if (otherAddress !== correctAnswer) options.add(otherAddress); if (ip !== correctAnswer && !options.has(ip)) options.add(ip);
        ...
        let optionsArray = Array.from(options); if (!optionsArray.includes(correctAnswer)) { optionsArray.pop(); optionsArray.push(correctAnswer); } optionsArray = optionsArray.slice(0, 4); shuffleArray(optionsArray);
        const explanationInfo = { generatorName: 'generatePortionExplanationHTML', args: [ip, mask, wildcardMask, networkAddr, broadcastAddr] };
        return { question, options: optionsArray, correctAnswer, explanation: explanationInfo };
