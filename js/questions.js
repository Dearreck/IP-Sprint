...
import {
    ...
    calculateNetworkAddress, calculateBroadcastAddress // Funciones de cálculo Red/Broadcast
} from './utils.js';
...
function generateClassQuestion() {
    try {
        ...
        // La respuesta correcta es la clase obtenida de getIpInfo
        const correct = info.class;
        // Genera la explicación HTML (tabla de rangos de clase, resaltando la correcta)
...
function generateTypeQuestion() {
    try {
        ...
        info = getIpInfo(ip);
        // Reintenta si la IP generada es de tipo inválido (N/A) o Loopback, APIPA, Broadcast
        ...
        // Genera la explicación (tabla de rangos privados, indicando si la IP es pública o privada)
...
function generateDefaultMaskQuestion() {
    try {
        ...
        // Genera la explicación (tabla Clase-Máscara, resaltando la clase de la IP)
...
function generateSelectClassQuestion() {
    try{
        ...
        const correct = correctIp; // La respuesta correcta es la IP que generamos primero
        // Explicación con la tabla de rangos de clase
...
function generateSelectPrivateIpQuestion() {
    try {
        ...
        // Genera una IP privada garantizada usando la función de utilidad
...
function generateRfc1918Question() {
    try {
        ...
        if (questionType === 0) {
            ...
            question = `El bloque privado ${rfcLinkHTML} <strong>${blockIdentifier}</strong> utiliza direcciones que originalmente pertenecen a la Clase:`;
        ...
        // Barajar opciones y asegurar correcta
        shuffleArray(options);
        if (options.length > 4) options = options.slice(0, 4);
        if (!options.includes(correctAnswer)) { options.pop(); options.push(correctAnswer); shuffleArray(options); }

        // Explicación con tabla RFC 1918
        explanation = generatePrivateRangeTableHTML(chosenBlock.blockStart);
        if (questionType === 2) {
            explanation += `<p style="font-size:0.9em; text-align:center; margin-top:5px;">Aunque el bloque <strong>${chosenBlock.blockId}</strong> es classless, las direcciones IP dentro de él caen en el rango original de la <strong>Clase ${chosenBlock.class}</strong>.</p>`;
        }
...
function generateSpecialAddressQuestion() {
    try {
        ...
        if (pickSpecial) {
            ...
            correctAnswer = chosenSpecial.description; // Usar descripción como respuesta
            ...
            if (questionFormat === 0) {
                ...
                if (options.length > 4) options = options.slice(0, 4);
                if (!options.includes(correctAnswer)) { options.pop(); options.push(correctAnswer); shuffleArray(options); }
                // Explicación específica para el tipo especial
                ...
                else { while (incorrectIps.size < 3) { let ip = generateRandomIp(); if (getIpInfo(ip).type !== 'Loopback' && getIpInfo(ip).type !== 'APIPA') { incorrectIps.add(ip); } } }
            ...
            // Asegurarse de que no sea especial por accidente
            if (info.type === 'Loopback' || info.type === 'APIPA' || info.type === 'Broadcast Limitado') {
                 return generateSpecialAddressQuestion(); // Reintentar si generó una especial
            }
            correctAnswer = info.type; // 'Pública' o 'Privada'
            const question = `La dirección IP <strong>${targetIp}</strong> es de tipo:`;
            let options = [correctAnswer];
            let incorrectOptions = specialAddresses.map(s => s.description); // Tipos especiales como incorrectos
            shuffleArray(incorrectOptions);
            options.push(...incorrectOptions.slice(0, 3));
            shuffleArray(options);
             if (options.length > 4) options = options.slice(0, 4);
             if (!options.includes(correctAnswer)) { options.pop(); options.push(correctAnswer); shuffleArray(options); }
            // Explicación normal (pública/privada)
...
function generateIdentifyNetworkPortionQuestion() {
    try {
        ...
        const question = `Dada la IP <strong>${ip}</strong> y su máscara por defecto (<strong>${info.defaultMask}</strong>), ¿cuál es la porción de red?`;
        const correctAnswer = portions.networkPortion || 'Ninguna'; // Respuesta correcta

        let options = new Set([correctAnswer]);

        // Generar opciones incorrectas
        // 1. Porción de Host de la misma IP
        if (portions.hostPortion && portions.hostPortion !== correctAnswer) { options.add(portions.hostPortion); }
        // 2. Porción de Red de otra IP
        let randomIpForPortion, randomInfoForPortion, incorrectNetworkPortion = null, portionAttempts = 0;
        do {
            randomIpForPortion = generateRandomIp(); randomInfoForPortion = getIpInfo(randomIpForPortion);
            if (randomInfoForPortion.defaultMask !== 'N/A') { incorrectNetworkPortion = getIpPortions(randomIpForPortion, randomInfoForPortion.defaultMask)?.networkPortion; }
        ...
        // 3. Porción de Host de otra IP
        let incorrectHostPortion = getIpPortions(randomIpForPortion, randomInfoForPortion.defaultMask)?.hostPortion;
        if (incorrectHostPortion && incorrectHostPortion !== correctAnswer && !options.has(incorrectHostPortion)) { options.add(incorrectHostPortion); }
...
function generateIdentifyHostPortionQuestion() {
    try {
        ...
        const question = `Dada la IP <strong>${ip}</strong> y su máscara por defecto (<strong>${info.defaultMask}</strong>), ¿cuál es la porción de host?`;
        const correctAnswer = portions.hostPortion || 'Ninguna'; // Respuesta correcta

        let options = new Set([correctAnswer]);

        // Generar opciones incorrectas
        // 1. Porción de Red de la misma IP
        if (portions.networkPortion && portions.networkPortion !== correctAnswer) { options.add(portions.networkPortion); }
        // 2. Porción de Host de otra IP
        let randomIpForPortion, randomInfoForPortion, incorrectHostPortion = null, portionAttempts = 0;
        do {
            randomIpForPortion = generateRandomIp(); randomInfoForPortion = getIpInfo(randomIpForPortion);
            if (randomInfoForPortion.defaultMask !== 'N/A') { incorrectHostPortion = getIpPortions(randomIpForPortion, randomInfoForPortion.defaultMask)?.hostPortion; }
        ...
        // 3. Porción de Red de otra IP
        let incorrectNetworkPortion = getIpPortions(randomIpForPortion, randomInfoForPortion.defaultMask)?.networkPortion;
        if (incorrectNetworkPortion && incorrectNetworkPortion !== correctAnswer && !options.has(incorrectNetworkPortion)) { options.add(incorrectNetworkPortion); }
...
function generateNetworkBroadcastAddressQuestion() {
    try {
        ...
        // Calcula la dirección de red y broadcast usando las utilidades
        const networkAddr = calculateNetworkAddress(ip, mask);
        const broadcastAddr = calculateBroadcastAddress(ip, mask);

        // Verifica que los cálculos fueron exitosos
        ...
        // 3. Dirección de red/broadcast de otra IP aleatoria (para más variedad)
        ...
        } while ((randomInfo2.class !== 'A' && randomInfo2.class !== 'B' && randomInfo2.class !== 'C' || randomInfo2.type === 'Loopback') && attempts2 < 50); // Busca otra IP válida
        if (randomInfo2.defaultMask !== 'N/A') { // Si tiene máscara default
            ...
            // Añade la dir de red incorrecta si es diferente y no está ya en opciones
            if (randomNetAddr && randomNetAddr !== correctAnswer && !options.has(randomNetAddr)) options.add(randomNetAddr);
            // Añade la dir de broadcast incorrecta si es diferente y no está ya en opciones
        ...
        // Añade la fórmula básica de cálculo
...
const associateQuestionGenerators = [
    ...
    generateNetworkBroadcastAddressQuestion // <-- Añadida
];
