...
import {
    ...
    calculateNetworkAddress, calculateBroadcastAddress, calculateWildcardMask // Importar utilidades de cálculo
} from './utils.js';
...
function generateRfc1918Question() {
    try {
        ...
        if (questionType === 0) {
            ...
            question = `El bloque privado ${rfcLinkHTML} <strong>${blockIdentifier}</strong> utiliza direcciones que originalmente pertenecen a la Clase:`; // Redacción mejorada
        ...
        if (questionType === 2) {
            explanation += `<p style="font-size:0.9em; text-align:center; margin-top:5px;">Aunque el bloque <strong>${chosenBlock.blockId}</strong> es classless, las direcciones IP dentro de él caen en el rango original de la <strong>Clase ${chosenBlock.class}</strong>.</p>`; // Explicación mejorada
        }
...
function generateNetworkBroadcastAddressQuestion() {
    try {
        ...
        // Calcula la dirección de red y wildcard
        const networkAddr = calculateNetworkAddress(ip, mask);
        const wildcardMask = calculateWildcardMask(mask);
        // Calcula el broadcast usando la dirección de red y la wildcard
        ...
        // Usa la nueva función de explicación con todos los datos
